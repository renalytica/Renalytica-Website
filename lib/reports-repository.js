...
(function (root, factory) {
  ...
  class ReportsRepository {
    ...
    /**
     * Sync in-memory catalog and localStorage with live backend Express server
     */
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data && Array.isArray(data.products) && data.products.length > 0) {
            let custom = {};
            try {
              custom = JSON.parse(localStorage.getItem('renalytica_custom_products') || '{}');
            } catch (e) {}

            data.products.forEach(p => {
              if (p && p.id) {
                custom[p.id] = { ...(custom[p.id] || {}), ...p };
                if (this.reports[p.id]) {
                  this.reports[p.id] = { ...this.reports[p.id], ...p };
                }
              }
            });

            try {
              localStorage.setItem('renalytica_custom_products', JSON.stringify(custom));
            } catch (e) {}

            if (typeof window !== 'undefined') {
              window.dispatchEvent(new CustomEvent('renalytica:catalog-synced', { detail: { products: data.products } }));
            }
          }
        }
      } catch (err) {
    ...
    /**
     * Get all reports (merged with local updates and customized pricing)
     */
    getAll() {
      ...
      try {
        if (typeof localStorage !== 'undefined') {
          ...
          Object.values(custom).forEach(cp => {
            ...
            let mergedItem = idx > -1 ? { ...list[idx], ...cp } : { ...cp };

            // Ensure pricing and regional_pricing accurately sync custom base_price / regional edits
            if (cp.regional_pricing || cp.base_price || cp.pricing) {
              const ngnPrice = (cp.regional_pricing && cp.regional_pricing.NGN) || (cp.currency === 'NGN' ? cp.base_price : null) || (cp.pricing && cp.pricing.singleNgn);
              const usdPrice = (cp.regional_pricing && cp.regional_pricing.USD) || (cp.currency === 'USD' ? cp.base_price : null) || (cp.pricing && cp.pricing.single);
              
              const baseSingleNgn = ngnPrice || (mergedItem.pricing && mergedItem.pricing.singleNgn) || 225000;
              const baseSingleUsd = usdPrice || (mergedItem.pricing && mergedItem.pricing.single) || Math.round(baseSingleNgn / 1500);

              mergedItem.pricing = {
                single: baseSingleUsd,
                singleNgn: baseSingleNgn,
                departmental: (cp.pricing && cp.pricing.departmental) || Math.round(baseSingleUsd * 3),
                departmentalNgn: (cp.pricing && cp.pricing.departmentalNgn) || Math.round(baseSingleNgn * 3),
                enterprise: (cp.pricing && cp.pricing.enterprise) || Math.round(baseSingleUsd * 6.333),
                enterpriseNgn: (cp.pricing && cp.pricing.enterpriseNgn) || Math.round(baseSingleNgn * 6.333),
                ...(mergedItem.pricing || {})
              };

              // Explicitly bind the active single prices
              mergedItem.pricing.single = baseSingleUsd;
              mergedItem.pricing.singleNgn = baseSingleNgn;

              mergedItem.regional_pricing = {
                NGN: baseSingleNgn,
                USD: baseSingleUsd,
                GHS: (cp.regional_pricing && cp.regional_pricing.GHS) || Math.round(baseSingleUsd * 15.5),
                KES: (cp.regional_pricing && cp.regional_pricing.KES) || Math.round(baseSingleUsd * 130),
                ...(cp.regional_pricing || {})
              };
            }
    ...
    getById(id) {
      ...
      return all.find(r => r.id === id || r.slug === id || r.sku === id) || this.reports[id] || null;
    }
    ...
    filter(criteria = {}) {
      ...
      if (criteria.format && criteria.format !== 'all') {
        results = results.filter(r => r.formats && r.formats.includes(criteria.format.toUpperCase()));
      }
      ...
      if (criteria.priceRange && criteria.priceRange !== 'all') {
        if (criteria.priceRange === 'under_2000') {
          results = results.filter(r => (r.pricing ? r.pricing.single : 0) < 2000);
        } else if (criteria.priceRange === '2000_3500') {
          results = results.filter(r => (r.pricing ? r.pricing.single : 0) >= 2000 && (r.pricing ? r.pricing.single : 0) <= 3500);
        } else if (criteria.priceRange === '3500_5000') {
          results = results.filter(r => (r.pricing ? r.pricing.departmental : 0) >= 3500 && (r.pricing ? r.pricing.departmental : 0) <= 5000);
        } else if (criteria.priceRange === 'over_5000') {
          results = results.filter(r => (r.pricing ? r.pricing.enterprise : 0) >= 5000);
        }
      }
      ...
      if (sortBy === 'newest') {
        ...
        results.sort((a, b) => (a.pricing ? a.pricing.single : 0) - (b.pricing ? b.pricing.single : 0));
      } else if (sortBy === 'price_desc') {
        results.sort((a, b) => (b.pricing ? b.pricing.single : 0) - (a.pricing ? a.pricing.single : 0));
      } else if (sortBy === 'pages') {
        results.sort((a, b) => (b.pageCount || 0) - (a.pageCount || 0));
      }
    ...
    addToCart(reportId, licenseType = 'departmental') {
      ...
      const price = (report.pricing && report.pricing[licenseType]) || (report.pricing && report.pricing.departmental) || 150;
      const priceNgn = (report.pricing && report.pricing[licenseType + 'Ngn']) || (report.regional_pricing && report.regional_pricing.NGN ? (licenseType === 'single' ? report.regional_pricing.NGN : report.regional_pricing.NGN * (licenseType === 'enterprise' ? 6.333 : 3)) : Math.round(price * 1500));

      if (existingIndex > -1) {
        cart[existingIndex].licenseType = licenseType;
        cart[existingIndex].price = price;
        cart[existingIndex].priceNgn = Math.round(priceNgn);
      } else {
        cart.push({
          id: report.id,
          sku: report.sku,
    ...
    updateLicense(reportId, licenseType) {
      ...
      if (item) {
        const report = this.getById(reportId);
        if (report) {
          const price = (report.pricing && report.pricing[licenseType]) || (report.pricing && report.pricing.single) || 150;
          const priceNgn = (report.pricing && report.pricing[licenseType + 'Ngn']) || (report.regional_pricing && report.regional_pricing.NGN ? (licenseType === 'single' ? report.regional_pricing.NGN : report.regional_pricing.NGN * (licenseType === 'enterprise' ? 6.333 : 3)) : Math.round(price * 1500));
          item.licenseType = licenseType;
          item.price = price;
          item.priceNgn = Math.round(priceNgn);
          this.saveCart(cart);
          return true;
        }
      }
    ...
    /**
     * Get cart subtotal in active or specified currency
     */
    getCartTotal(curr = null) {
      if (!curr && typeof localStorage !== 'undefined') {
        curr = localStorage.getItem('renalytica_currency') || 'USD';
      }
      const cart = this.getCart();
      if (curr === 'NGN') {
        return cart.reduce((total, item) => total + (item.priceNgn || Math.round((item.price || 0) * 1500)), 0);
      }
