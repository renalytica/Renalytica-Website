  ...
  class FlutterwaveCheckoutManager {
    ...
    handleSuccessfulPayment(response, reportIds, licenseType, onSuccess) {
      ...
      try {
        ...
        // Post order to persistent backend
        fetch('/api/orders', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            order_id: purchaseRecord.txRef,
            customer_name: (window.RenalyticaAuth && window.RenalyticaAuth.getUser()?.fullName) || 'Institutional Client',
            customer_email: (window.RenalyticaAuth && window.RenalyticaAuth.getUser()?.email) || 'client@renalytica.com',
            customer_org: (window.RenalyticaAuth && window.RenalyticaAuth.getUser()?.organization) || 'Commercial Advisory',
            product_title: reportIds.join(', '),
            amount: purchaseRecord.amount,
            currency: purchaseRecord.currency,
            status: 'completed',
            payment_method: 'Online Card (Flutterwave)',
            license: licenseType
          })
        }).catch(e => console.warn('Order sync to backend notice:', e.message));
      } catch (e) {
