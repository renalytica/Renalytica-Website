...
(function () {
  ...
  function executeEntranceReveal() {
    ...
    // 2. Animate content elements away with GSAP or CSS fallback
  ...
  function initGateway() {
    ...
    // Setup Enter button and Skip trigger
    enterBtn = document.getElementById('gateway-enter-btn');
    if (enterBtn) {
      enterBtn.addEventListener('click', executeEntranceReveal);
      enterBtn.addEventListener('touchend', executeEntranceReveal);
    }

    const skipBtn = document.getElementById('gateway-skip-btn');
    if (skipBtn) {
      skipBtn.addEventListener('click', executeEntranceReveal);
      skipBtn.addEventListener('touchend', executeEntranceReveal);
    }

    // Keyboard support: pressing Enter triggers access
    window.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !isTransitionComplete) {
    ...
    // GSAP Intro choreography for the gateway content
    if (typeof gsap !== 'undefined') {
      gsap.from('.gateway-telemetry-badge', { y: -20, opacity: 0, duration: 0.8, delay: 0.2, ease: 'power2.out' });
      gsap.from('.gateway-logo-wrapper', { scale: 0.8, opacity: 0, duration: 1.2, delay: 0.4, ease: 'back.out(1.4)' });
      gsap.from('.gateway-tagline', { y: 20, opacity: 0, duration: 0.8, delay: 0.7, ease: 'power2.out' });
      gsap.from('.gateway-btn-container', { y: 30, opacity: 0, duration: 0.9, delay: 0.9, ease: 'power2.out' });
    }
  }
